...
def create_article(article: Article):
    ...
    cur = conn.cursor()
    cur.execute(
        "INSERT INTO articles_mosquito (title, content, author_id, created_at) VALUES (%s, %s, %s, %s) RETURNING id;",
...
def read_article(id: int):
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute("SELECT * FROM articles_mosquito WHERE id = %s;", (id,))
...
def update_article(id: int, article: Article):
    ...
    cur = conn.cursor()
    cur.execute(
        "UPDATE articles_mosquito SET title = %s, content = %s WHERE id = %s;",
...
def delete_article(id: int):
    ...
    cur.execute("DELETE FROM articles_mosquito WHERE id = %s;", (id,))
    conn.commit()
    cur.close()
    conn.close()
